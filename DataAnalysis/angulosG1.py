...
# Directorio donde se encuentran las carpetas de Tiro
directorio_tiros = "Frames/Jugador3Sec3"

# Iterar sobre las carpetas de Tiro
for tiro_carpeta in os.listdir(directorio_tiros):
    directorio_tiro = os.path.join(directorio_tiros, tiro_carpeta)
    if os.path.isdir(directorio_tiro):
        # Construir la ruta a la subcarpeta de datos
        directorio_datos_tiro = os.path.join(directorio_tiro, 'joints', '3D_Region_no_nan_El_2')

        # Crear el directorio de ángulos específico para esta carpeta de Tiro
        directorio_angulos_tiro = os.path.join(directorio_tiro, 'joints', 'angulos')
        os.makedirs(directorio_angulos_tiro, exist_ok=True)

        # Iterar sobre los archivos en la subcarpeta de datos
        for archivo in os.listdir(directorio_datos_tiro):
            if archivo.endswith(".txt"):
                ruta_completa = os.path.join(directorio_datos_tiro, archivo)
                ...
                try:
                    ...
                    ruta_archivo_angulos = os.path.join(directorio_angulos_tiro, nombre_archivo_angulos)
